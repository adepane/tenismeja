        ...
        <div class="bookmark-wrapper d-flex align-items-center">
            <ul class="nav navbar-nav d-xl-none">
                <li class="nav-item"><a class="nav-link menu-toggle"
                        href="#"><i class="fas fa-bars fa-lg"></i></a></li>
            </ul>
        </div>
        <ul class="nav navbar-nav align-items-center ms-auto">
            <li class="nav-item d-none d-lg-block"><a class="nav-link nav-link-style"><i
                        class="fas fa-moon fa-lg"></i></a></li>
            {{-- <li class="nav-item dropdown dropdown-notification me-25"><a class="nav-link"
                    href="#"
                    data-bs-toggle="dropdown"><i class="fas fa-bell fa-lg"></i><span
            ...
            <li class="nav-item dropdown dropdown-user"><a class="nav-link dropdown-toggle dropdown-user-link"
                    ...
                    aria-haspopup="true"
                    aria-expanded="false">
                    <div class="user-nav d-sm-flex d-none"><span class="user-name fw-bolder">@if (!is_null(Session::get('orig_user'))) <span class="text-danger"> Login As </span>@endif{!!$user->name !!}</span><span class="user-status">{!! $user?->getPermission?->name !!}</span></div>
                    <span class="avatar">
                        <img class="round"
                ...
                <div class="dropdown-menu dropdown-menu-end"
                    ...
                    <a class="dropdown-item ajax-load" href="{!!url(Core::links('users/profile/'.Auth::user()->id))!!}"
                        id="editprofileuser"><i class="fas fa-user-alt text-muted align-middle mr-1"></i>
                        Profile</a>
                    @if (!is_null(Session::get('orig_user')))
                    <a class="dropdown-item ajax-load" href="{!!url('switchback')!!}" id="switchback"><i
                            class="fas fa-sign-in-alt text-muted align-middle mr-1"></i>
                        Back to {!!Core::switchUser(Session::get('orig_user'))!!}</a>
                    @endif
                    <a class="dropdown-item"
                        href="{!!route('logout')!!}"><i class="fas fa-sign-out-alt text-muted align-middle mr-1"></i> Logout</a>
                </div>
            </li>
        </ul>
    </div>
