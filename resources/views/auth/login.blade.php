...
<html class="loading" lang="en" data-textdirection="ltr">
  ...
  <head>
    ...
    <link rel="stylesheet" href="{!!asset('/css/plugins/extensions/ext-component-toastr.css')!!}">
    <!-- END: Vendor CSS-->
  ...
  <body class="vertical-layout vertical-menu-modern blank-page navbar-floating footer-static  " data-open="click" data-menu="vertical-menu-modern" data-col="blank-page">
            ...
            <div class="auth-inner row m-0">
              ...
              <div class="d-flex col-lg-4 align-items-center auth-bg px-2 p-lg-5">
                <div class="col-12 col-sm-8 col-md-6 col-lg-12 px-xl-2 mx-auto">
                  <h2 class="card-title fw-bold mb-1">{!!config('app.name')!!}</h2>
                  <p class="card-text mb-2">Get more things done with Login platform.</p>
    ...
    <!-- END: Theme JS-->
    <!-- BEGIN: Page JS-->
    {{-- <script src="{!!asset('js/scripts/pages/auth-login.min.js')!!}"></script> --}}
    ...
    <script>
      ...
      loginForm.addEventListener('submit', (e) => {
        ...
        if (email != '' || password != '') {
            authCredential({'email': email, 'password': password, 'rememberMe': rememberMe});
        }
        return;
      });

      const authCredential = async (props) => {
        try {
            const testCredential = await axios.post("{!! url('/login') !!}", {
                'email': props.email,
                'password': props.password,
                'remember_me': props.rememberMe
            });
            console.log(testCredential);
            if (testCredential.data.success) {
                toastr['success'](
                  'You have successfully logged',
                  '👋 Welcome '+ testCredential.data.data.user.name +'!',
                );
                await setTimeout(() => { window.location.href=testCredential.data.data.route_to; }, 500);
            } else {
                Swal.fire({
                icon: 'error',
                title: 'Oops...',
                text: testCredential.data.message,
              })  
            }
